...
var shortcut,  Cdisplay, Ctop, Cleft, Copacity ;
var msg="Please type the path where the image 'pixelperfect.jpg or pixelperfect.png' is located";
var msgError="pixelperfect image not found,please select new path or url.";
...
function saveValue(llave, valor) {
	...
	case "Cpatch":
		...
		break;
	default:
		console.log("falta meter: " + llave + " -> " + valor);
	}
}
...
function readValue(llave) {
	...
	case "Cpatch":
		...
		break;
	default:
		console.log(" no se pudo dar este valor -> " + llave);
	}
}
...
function create_shortcuts (){
	...
	shortcut.add("Shift+home",		function() {set_opacity('plus');	return false;});
	shortcut.add("Shift+end",		function() {set_opacity('minus');	return false;});
...
function setup(){
	...
	if (readValue('Cpatch')) {image_check(readValue('Cpatch'),0);} else {ppDialog(msg);}
}	
